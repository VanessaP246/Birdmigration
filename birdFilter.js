...
//   // Partition neu berechnen – nur verfügbare Leaves zählen
//   root.sum(function(node) {
//     if (!node) return 0;
//     if (!node.children || node.children.length === 0) {
//       const name = node.name || (node.data && node.data.name);
//       return (available && available.species && available.species.has(name))
//         ? (node.value || (node.data && node.data.value) || 1)
//         : 0;
//     }
//     return 0;
//   });
...
//   // Zoom-Zustand wiederherstellen nach Partition-Neuberechnung
//   if (currentRoot === root) {
//     // Kein Zoom aktiv – alle Segmente normal anzeigen mit neuen x0/x1
//     const xScale2 = d3.scaleLinear().domain([0, 2 * Math.PI]).range([0, 2 * Math.PI]);
    
//     paths.transition().duration(400).attr("d", arc);
...
//   } else {
//     // Zoom aktiv – zoomTo mit aktuellem Target neu aufrufen
//     // damit die neuen x0/x1 korrekt auf den gezoomten Bereich gemappt werden
//     const savedRoot = currentRoot;
//     currentRoot = root; // kurz zurücksetzen damit zoomTo nicht abbricht
//     zoomTo(savedRoot);
//   }
// };
...
function buildBirdDataFromRoutes() {
  ...
  const orderMap = {};
  for (const r of allRoutes) {
    const order = r.order || '', family = r.family || '',
          genus = r.genus || '', species = r.species || '';
    if (!order || !family || !genus || !species) continue;
    if (!orderMap[order]) { orderMap[order] = { name: order, children: [], _fm: {} }; root.children.push(orderMap[order]); }
    const oN = orderMap[order];
    if (!oN._fm[family]) { oN._fm[family] = { name: family, children: [], _gm: {} }; oN.children.push(oN._fm[family]); }
    const fN = oN._fm[family];
    if (!fN._gm[genus]) { fN._gm[genus] = { name: genus, children: [], _sm: {} }; fN.children.push(fN._gm[genus]); }
    const gN = fN._gm[genus];
    if (!gN._sm[species]) { gN._sm[species] = { name: species, value: 0 }; gN.children.push(gN._sm[species]); }
    gN._sm[species].value += r.points.length;
  }
  function clean(n) { delete n._fm; delete n._gm; delete n._sm; if (n.children) n.children.forEach(clean); }
  clean(root);
  return root;
}
...
function initBirdFilter() {
  ...
  // Hierarchie – IMMER alle Daten, nie gefiltert
  const birdData = buildBirdDataFromRoutes() || BIRD_DATA;
  const root = d3.hierarchy(birdData)
    .sum(d => d.value || 0)
    .sort((a, b) => b.value - a.value);

  // Partition einmalig berechnen – wird NIE neu berechnet
  d3.partition().size([2 * Math.PI, radius])(root);
  ...
  const arc = d3.arc()
    ...
    .innerRadius(d => xScale(d)).outerRadius(d => x1Scale(d))
    .padAngle(0.012).padRadius(radius / 2);

  // Farben
  const colorMap = {};
  root.children.forEach((d, i) => { colorMap[d.data.name] = COLOR_GROUPS[i % COLOR_GROUPS.length]; });

  // Globale Hilfsfunktion für mapRoutes.js
  try {
    window._birdColorMap = colorMap;
    window.getBirdLightColor = function(orderName) {
      if (!orderName) return null;
      const g = (window._birdColorMap || {})[orderName];
      if (g) return g[0];
      let h = 0;
      for (let i = 0; i < orderName.length; i++) { h = ((h << 5) - h) + orderName.charCodeAt(i); h |= 0; }
      return COLOR_GROUPS[Math.abs(h) % COLOR_GROUPS.length][0];
    };
  } catch(e) {}
  ...
  function getColor(d) {
    ...
    const grp = colorMap[n.data.name] || COLOR_GROUPS[0];
    return grp[d.depth - 1] || grp[grp.length - 1];
  }
  ...
  const tooltip = d3.select(container).append('div')
    ...
    .style('font-size', '0.75rem').style('pointer-events', 'none').style('opacity', 0)
    .style('max-width', '160px').style('line-height', '1.4');
  ...
  function fitLabel(node, text, maxW) {
    ...
    while (lo <= hi) {
      const mid = (lo + hi) >> 1;
      el.textContent = text.slice(0, mid) + '…';
      if (el.getComputedTextLength() <= maxW) { fit = mid; lo = mid + 1; } else hi = mid - 1;
    }
    el.textContent = fit > 0 ? text.slice(0, fit) + '…' : '';
  }
  ...
  const paths = g.selectAll('path')
    ...
    .on('mouseover', function(event, d) {
      d3.select(this).attr('opacity', 0.75);
      tooltip.html(`<strong style="font-size:0.85rem">${d.data.name}</strong><br>${DEPTH_LABEL[d.depth]}<br>${d.value.toLocaleString()} data points`).style('opacity', 1);
    })
    .on('mousemove', function(event) {
      const rect = container.getBoundingClientRect();
      const cx = event.clientX - rect.left, cy = event.clientY - rect.top;
      const el = tooltip.node();
      let lx = cx + 12, ly = cy - 10;
      if (el) {
        const tw = el.offsetWidth, th = el.offsetHeight;
        if (lx + tw > rect.width - 8) lx = cx - tw - 12;
        if (lx < 8) lx = 8;
  ...
  const labels = g.selectAll('text')
    ...
    .each(function(d) { placeLabel(d3.select(this), d, xScale(d), x1Scale(d), d.x0, d.x1); });

  function placeLabel(node, d, ir, or_, a0, a1) {
    const midR   = (ir + or_) / 2;
    const arcLen = (a1 - a0) * midR;
    if (arcLen < 20) { node.text(''); return; }
    const midA = (a0 + a1) / 2 - Math.PI / 2;
    let rot = (midA + Math.PI / 2) * 180 / Math.PI;
    if (rot > 90 && rot < 270) rot += 180;
    node.attr('transform', `translate(${Math.cos(midA) * midR},${Math.sin(midA) * midR}) rotate(${rot})`);
    fitLabel(node, d.data.name, Math.max(0, arcLen - 6));
  }
  ...
  const centerArrow = g.append('text')
    .attr('text-anchor', 'middle').attr('y', innerR * 0.35)
  ...
  function applyAvailability() {
    ...
    // Werte neu summieren – nicht verfügbare Arten auf 0
    root.sum(d => {
      if (d.children) return 0;
      if (!av) return d.value || 0;
      return av.species.has(d.name) ? (d.value || 0) : 0;
    });

    // Partition neu berechnen → Lücken schließen sich
    d3.partition().size([2 * Math.PI, radius])(root);

    // Aktuellen Zoom-Zustand beibehalten
    const target = currentRoot;
    const x0 = target.x0, dx = target.x1 - target.x0;
    const xScale2 = target === root
      ? d3.scaleLinear().domain([0, 2 * Math.PI]).range([0, 2 * Math.PI])
      : d3.scaleLinear().domain([x0, x0 + dx]).range([0, 2 * Math.PI]);
    const baseDepth = Math.max(target.depth + 1, 1);

    paths.transition().duration(400).attrTween('d', function(d) {
      const visible = target === root ? true : (d !== target && isDescendant(target, d));
      d3.select(this)
        .attr('opacity', (visible && d.value > 0) ? 1 : 0)
        .attr('pointer-events', (visible && d.value > 0) ? 'all' : 'none');

      const newArc = d3.arc()
        .startAngle(dd => xScale2(Math.max(x0, Math.min(x0 + dx, dd.x0))))
        .endAngle(dd   => xScale2(Math.max(x0, Math.min(x0 + dx, dd.x1))))
        .innerRadius(dd => innerR + Math.max(0, dd.depth - baseDepth) * ringW)
        .outerRadius(dd => innerR + (Math.max(0, dd.depth - baseDepth) + 1) * ringW)
        .padAngle(0.012).padRadius(radius / 2);
      return () => newArc(d);
    });

    setTimeout(() => updateLabels(target, xScale2, baseDepth), 420);
  }
  ...
  function zoomTo(target) {
    ...
    const x0 = target.x0, dx = target.x1 - target.x0;
    const xScale2 = d3.scaleLinear().domain([x0, x0 + dx]).range([0, 2 * Math.PI]);
    const minDepth = target.depth, baseDepth = Math.max(minDepth + 1, 1);
    ...
    paths.transition().duration(600).attrTween('d', function(d) {
      ...
      const newArc = d3.arc()
        ...
        .padAngle(0.012).padRadius(radius / 2);
      return function() {
        const show = target === root ? true : visible;
        d3.select(this).attr('opacity', show ? 1 : 0).attr('pointer-events', show ? 'all' : 'none');
        return newArc(d);
      };
    });
  ...
  function updateCenter(target) {
    const isRoot = target === root;
    centerCircle.transition().duration(400).attr('fill', isRoot ? '#1a1d22' : getColor(target));
    centerText.text(isRoot ? 'All birds' : target.data.name)
      .attr('font-size', isRoot ? '0.75rem' : '0.7rem')
      .attr('y', isRoot ? 0 : -(innerR * 0.2));
    centerArrow.text(isRoot ? '' : '↩');
  }

  function updateLabels(target, xScale2, baseDepth) {
    const x0 = target.x0, dx = target.x1 - target.x0;
    labels.each(function(d) {
      const node    = d3.select(this);
      const visible = d !== target && isDescendant(target, d);
      ...
      const relD  = d.depth - baseDepth;
      const a0    = xScale2(Math.max(x0, Math.min(x0 + dx, d.x0)));
      const a1    = xScale2(Math.max(x0, Math.min(x0 + dx, d.x1)));
      const ir    = innerR + Math.max(0, relD) * ringW;
      const or_   = ir + ringW;
      placeLabel(node, d, ir, or_, a0, a1);
    });
  }
